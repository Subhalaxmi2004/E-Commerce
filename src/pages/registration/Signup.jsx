...
const sign = async () => {
  setLoading(true);
  try {
    const users = await createUserWithEmailAndPassword(auth, email, fullname.password);
    console.log(users);
    const user = {
      name:fullname.fname,
      uid:users.user.uid,
 ...
 return(
    ...
    <form onSubmit={onSubmit}>
        ...
        <span className="cp">{confirm}</span>
         ...
         <button className="signup_submit"type="submit" onClick={final}>Create Your Account</button>
        </form>
